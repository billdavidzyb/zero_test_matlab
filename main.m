...
L_n(6) = Link('d', d_n(6) + 0.1, 'a', a_n(6), 'alpha', p_n(6));
...
for i = 1:sample
    for j = 1:4
        ...
        jac_mat(j,1:3,1:6) = tmp_jac(4:6,1:6);  %%%%%%%%%%%%%%%%%%%%%%
    end
    ...
    a1 = - a1;
    a2 = coord(1,4) - coord(1,1);
    c1 = - c1;
    c2 = coord(2,4) - coord(2,1);
    e1 = - e1;
    e2 = coord(3,4) - coord(3,1);
    B1 = - B1;
    B2 = jac_mat(4,1,:) - jac_mat(1,1,:);
    D1 = - D1;
    D2 = jac_mat(4,2,:) - jac_mat(1,2,:);
    F1 = - F1;
    F2 = jac_mat(4,3,:) - jac_mat(1,3,:);
    b(i+sample) = - (a1 * a2 + c1 * c2 + e1 * e2);
    A1(i+sample,:) = a1 * B2 + a2 * B1 + c1 * D2 + c2 * D1 + e1 * F2 + e2 * F1;
    
    a1 = coord(1,3) - coord(1,4);
    a2 = - a2;
    c1 = coord(2,3) - coord(2,4);
    c2 = - c2;
    e1 = coord(3,3) - coord(3,4);
    e2 = - e2;
    B1 = jac_mat(3,1,:) - jac_mat(4,1,:);
    B2 = - B2;
    D1 = jac_mat(3,2,:) - jac_mat(4,2,:);
    D2 = - D2;
    F1 = jac_mat(3,3,:) - jac_mat(4,3,:);
    F2 = - F2;
    b(i+2*sample) = - (a1 * a2 + c1 * c2 + e1 * e2);
    A1(i+2*sample,:) = a1 * B2 + a2 * B1 + c1 * D2 + c2 * D1 + e1 * F2 + e2 * F1;
    
    a1 = - a1;
    a2 = coord(1,2) - coord(1,3);
    c1 = - c1;
    c2 = coord(2,2) - coord(2,3);
    e1 = - e1;
    e2 = coord(3,2) - coord(3,3);
    B1 = - B1;
    B2 = jac_mat(2,1,:) - jac_mat(3,1,:);
    D1 = - D1;
    D2 = jac_mat(2,2,:) - jac_mat(3,2,:);
    F1 = - F1;
    F2 = jac_mat(2,3,:) - jac_mat(3,3,:);
    b(i+3*sample) = - (a1 * a2 + c1 * c2 + e1 * e2);
    A1(i+3*sample,:) = a1 * B2 + a2 * B1 + c1 * D2 + c2 * D1 + e1 * F2 + e2 * F1;
end
A = A1(:,1:5);
[Q,R] = qr(A);
result = pinv(R)*Q'*b
